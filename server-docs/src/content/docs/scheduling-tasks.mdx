...
## ECS: TickingSystem Alternative

For per-world or per-entity timed logic, use ECS `TickingSystem` instead of scheduled executors. Systems run on the world thread every tick (usually 20 ticks/second).

### Basic TickingSystem

```kotlin
class CooldownSystem : TickingSystem<EntityStore>() {
    override fun tick(deltaTime: Float, tick: Int, store: Store<EntityStore>) {
        val buffer = store.takeCommandBuffer()

        for (chunk in store.getChunks(CooldownComponentType)) {
            for (i in 0 until chunk.size()) {
                val cooldown = chunk.getComponent(i, CooldownComponentType)

                cooldown.remaining -= deltaTime
                if (cooldown.remaining <= 0) {
                    // Cooldown expired - remove component
                    buffer.removeComponent(chunk.getRef(i), CooldownComponentType)
                }
            }
        }
...
```kotlin
class DespawnTimerSystem : EntityTickingSystem<EntityStore>() {
    override fun getQuery() = DespawnComponentType
    ...
    override fun tick(
        ...
        val despawn = chunk.getComponent(index, DespawnComponentType)

        despawn.timeRemaining -= deltaTime
        if (despawn.timeRemaining <= 0) {
...
class MyPlugin(init: JavaPluginInit) : JavaPlugin(init) {
    override fun setup() {
        ...
        getEntityStoreRegistry().registerSystem(CooldownSystem())
        getEntityStoreRegistry().registerSystem(DespawnTimerSystem())
    }
}
```
...
| Use Case | Solution |
|----------|----------|
| Server-wide timers (broadcasts, saves) | `SCHEDULED_EXECUTOR` |
| Per-world game logic | `TickingSystem` |
| Per-entity updates (cooldowns, AI) | `EntityTickingSystem` |
| One-time delayed action | `SCHEDULED_EXECUTOR.schedule()` |
| Component change reactions | `RefChangeSystem` |
...
```kotlin
class GameManager(private val plugin: JavaPlugin) {
    // Server-wide match timer (runs regardless of world state)
    private var matchTimer: ScheduledFuture<*>? = null

    // ECS system for per-entity game logic
    private val gameSystem = GameTickSystem()

    fun start() {
        // Register ECS system for world-thread processing
        plugin.getEntityStoreRegistry().registerSystem(gameSystem)

        // Start server-wide timer
        matchTimer = HytaleServer.SCHEDULED_EXECUTOR.scheduleAtFixedRate({
            // This runs on executor thread - use for broadcasts, checks
            checkMatchEnd()
        }, 0, 1, TimeUnit.SECONDS)
    }
}
...
- [System Types](/system-types) - Complete system type reference
- [Advanced ECS](/advanced-ecs) - Advanced system patterns
